...
const worker = new Worker(
  ...
  async (job) => {
    try {
      ...
      // Extrahujeme a analyzujeme obrázky pomocí AI
      let extractedCount = 0;
      let analyzer: VisionAnalyzer | null = null;
      
      // Zkusíme vytvořit analyzer, pokud máme API klíč
      if (process.env.DEEPSEEK_API_KEY) {
        try {
          analyzer = new VisionAnalyzer(process.env.DEEPSEEK_API_KEY);
          console.log('🤖 VisionAnalyzer initialized with DeepSeek API');
        } catch (error) {
          console.error('❌ Failed to initialize VisionAnalyzer:', error);
        }
      } else {
        console.log('⚠ DEEPSEEK_API_KEY not found, using fallback simulation');
        return await fallbackSimulation(listingId, zipUrl, title, price);
      }
      
      for (let idx = 0; idx < Math.min(imageEntries.length, 20); idx++) {
        ...
        try {
          console.log(`📸 Processing image ${idx + 1}/${Math.min(imageEntries.length, 20)}...`);
          ...
          // Ulož do databáze s výsledky analýzy
          await prisma.listingMedia.create({
            data: {
              url,
              originalName: entry.entryName,
              category: analysis.roomType,  // 🎯 ROZTŘÍDĚNO PODLE MÍSTNOSTI
              listingId,
              processingStatus: 'DONE',
              processedAt: new Date(),
              ...
              sortOrder: idx,
              // Nově přidaná pole pro popisky
              aiDescription: analysis.description,
              aiCaption: analysis.suggestedCaption,
            }
          });

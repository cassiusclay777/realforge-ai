...
export class PoskiDataMapper {
  ...
  static mapListingToAdvert(listing: RealForgeListing, sellerId: number): PoskiAdvertData {
    ...
    return {
      ...
      locality_district: addressParts.district ?? '',
  ...
  /**
   * Mapuje REALFORGE media na PoskiREAL PhotoData (sync – data zůstávají placeholder bez fetchu)
   */
  static mapMediaToPhotos(media: RealForgeListing['media'] = []): PoskiPhotoData[] {
    return media.map((item, index) => ({
      ...
      data: this.getBase64FromUrl(item.url),
      alt: item.originalName || `Foto ${index + 1}`
    }));
  }

  /**
   * Mapuje REALFORGE media na PoskiREAL PhotoData s reálným načtením obrázků (async).
   * @param media Pole media z listingu
   * @param baseUrl Základ URL pro relativní cesty (např. process.env.NEXTAUTH_URL)
   */
  static async mapMediaToPhotosAsync(
    media: RealForgeListing['media'] = [],
    baseUrl: string = ''
  ): Promise<PoskiPhotoData[]> {
    const results: PoskiPhotoData[] = [];
    for (let index = 0; index < media.length; index++) {
      const item = media[index];
      const data = await this.getBase64FromUrlAsync(item.url, baseUrl);
      results.push({
        main: item.isFeatured ? 1 : 0,
        order: item.sortOrder || index + 1,
        photo_rkid: `RF_PHOTO_${item.url.split('/').pop()?.split('.')[0] || index}`,
  ...
  /**
   * Convert image URL to base64 (sync placeholder – pro zpětnou kompatibilitu)
   */
  private static getBase64FromUrl(url: string): string {
    console.warn('Base64 conversion not implemented for URL (use mapMediaToPhotosAsync):', url);

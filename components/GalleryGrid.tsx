                        ...
                        if (parent) {
                          ...
                          placeholder.innerHTML = `
                            <div class="text-center p-8">
                              ...
                              <p class="text-sm text-gray-400 mt-2">${getCategoryLabel(featuredMedia.category)}</p>
                            </div>
                          `;
                    ...
                    <div className="absolute inset-0 flex items-center justify-center">
                      <div className="text-center p-8">
                        ...
                        <p className="text-amber-200 text-base font-sans tracking-tight">
                          {getCategoryLabel(featuredMedia.category)}
                        </p>
                      </div>
                    </div>
                  </>
                ...
                >
                  <CardContent className="p-0">
                    <div className="aspect-square relative">
                      ...
                      {/* Hover overlay s AI popisky */}
                      <div className="absolute inset-0 bg-black/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col items-center justify-center p-4 overflow-hidden">
                        <div className="text-center min-w-0 w-full">
                          <p className="text-white font-bold text-lg font-sans tracking-tight mb-2">
                            {getCategoryLabel(item.category)}
                          </p>
                          
                          {/* AI popisek */}
                          {item.aiCaption && (
                            <p className="text-white/90 text-sm font-sans tracking-tight line-clamp-2 mb-2">
                              &quot;{item.aiCaption}&quot;
                            </p>
                          )}
                          {/* AI popis (detailnější) – max 3 řádky */}
                          {item.aiDescription && (
                            <p className="text-white/80 text-xs font-sans tracking-tight line-clamp-3 break-words">
                              {item.aiDescription}
                            </p>
                          )}
                          ...
                          {item.aiSaliencyScore && (
                            <div className="mt-2">
                              <Badge className="bg-white/20 text-white border-0 text-xs font-sans tracking-tight">
                                AI Confidence: {Math.round(item.aiSaliencyScore * 100)}%
                              </Badge>
                            </div>
                          )}
                          ...
                          {item.aiTags.length > 0 && (
                            <div className="mt-2 flex flex-wrap justify-center gap-1">
                              {item.aiTags.slice(0, 3).map((tag) => (
                                <span key={`${item.id}-tag-${tag}`} className="text-white/70 text-xs bg-white/10 px-2 py-0.5 rounded">
                                  {tag}
                                </span>
                              ))}
                            </div>
                      ...
                      {/* AI popisek na spodku fotky – jeden řádek, ořezaný */}
                      {item.aiCaption && (
                        <div className="absolute bottom-0 left-0 right-0 p-2 bg-gradient-to-t from-black/80 to-transparent overflow-hidden">
                          <p className="text-white text-xs font-sans tracking-tight line-clamp-1 break-words truncate">
                            {item.aiCaption}
                          </p>
                        </div>
                      )}

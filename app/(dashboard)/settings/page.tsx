...
export default function SettingsPage() {
  ...
  // Load profile from API; fallback to session name/email if API fails (e.g. missing session.user.id).
  useEffect(() => {
    if (status !== "authenticated") return;
    let cancelled = false;
    fetch("/api/user", { credentials: "same-origin" })
      .then((res) => {
        if (!res.ok) return res.json().then((d) => { throw new Error(d?.error ?? "Nepodařilo se načíst profil"); });
        return res.json();
      })
      .then((data) => {
        if (!cancelled) {
          setProfile({
            name: data.name ?? "",
            email: (data.email ?? (sessionData?.user as { email?: string })?.email) ?? "",
            phone: data.phone ?? "",
            office: data.office ?? "",
            role: data.role ?? "",
          });
      ...
      .catch((err) => {
        if (!cancelled) {
          setProfileLoadError(err.message ?? "Chyba načtení");
          const u = sessionData?.user;
          if (u) {
            setProfile((p) => ({
              ...p,
              name: (u.name as string) ?? p.name,
              email: ((u as { email?: string }).email ?? p.email) || "",
            }));
          }
        }
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setProfileLoadError(e instanceof Error ? e.message : "Chyba při ukládání");
    } finally {
  ...
  const handleInputChange = () => {
    ...
    if (profileLoadError) setProfileLoadError(null);
  };
            ...
            <div className="space-y-4">
              ...
              <div className="pt-4 space-y-2">
                ...
                <Button onClick={handleSave} disabled={profileSaving}>
                  {profileSaving ? "Ukládám…" : "Uložit změny profilu"}
                </Button>
              </div>
            </div>
          </div>
